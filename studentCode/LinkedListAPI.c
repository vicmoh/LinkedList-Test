...
char* toString(List list){
    ...
    if(list.head == NULL || list.tail == NULL){
        char* feedback = malloc(sizeof(char) + strlen("No List.\n"));
    ...
    //iterate through the list
    while (currentNode != NULL) {
        total = total + strlen(list.printData(currentNode->data)) + 9 ;
        string = realloc(string, sizeof(string) + total);
        strcat(string, list.printData(currentNode->data));
        if(currentNode->next != NULL){
            strcat(string, "\n");
        }//end if
        currentNode = currentNode->next;
...
void* findElement(List list, bool (*customCompare)(const void* first,const void* second), const void* searchRecord){
    ...
    while(currentNode != NULL){
        if(customCompare(currentNode->data, searchRecord)){
